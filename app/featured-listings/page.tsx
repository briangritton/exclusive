...
export default function FeaturedListingsPage() {
  ...
  useEffect(() => {
    ...
    const loadWidget = () => {
      ...
      if (ihfKestrel && ihfKestrel.render && containerRef.current) {
        try {
          // Render SEO page - iHomefinder auto-detects /featured-listings from URL
        ...
        if (attempts < maxAttempts) {
          ...
          setError('Featured properties temporarily unavailable');
          setLoading(false);
        }
      }
    };
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Exclusive Properties</h1>
        <p className="mt-2 text-gray-600">
          Hand-picked luxury homes curated just for you
        </p>
      </div>
      ...
      {loading && !error && (
        <div className="text-center py-12">
          <div className="text-gray-500">Loading exclusive properties...</div>
        </div>
      )}
      ...
      {/* Widget container */}
      <div className="ihf-widget-isolation" ref={containerRef} id="featured-listings-container"></div>
    </div>
  );
}
